...
func (annoucementRepo PsqlAnnouncementAdapter) FindStudentAnnouncements(studentId string) ([]annoucement_domain.Announcement, error) {
	...
	LEFT JOIN public.students ON public.students.section_id = public.announcements.section_id
	WHERE public.announcements.section_id IS NULL OR public.students.user_id = $1`, studentId)
	if err != nil {
		return announcements, err
	}
