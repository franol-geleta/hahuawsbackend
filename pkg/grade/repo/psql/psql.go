...
func (repo GradePsqlRepoAdapter) FindGradesByUser(userId string) ([]grade_domain.Grade, error) {
	...
	public.users.first_name, public.users.last_name, public.sections.name, public.classes.name,
	...
	for rows.Next() {
		...
		var _mid = sql.NullFloat64{}
		var _final = sql.NullFloat64{}
		err = rows.Scan(&grade.ID, &_ass, &_mid, &_final, &grade.User.ID, &grade.CanReview, &grade.ReviewRequested, &grade.Sunmitted,
			&grade.User.FirstName, &grade.User.LastName,
			&grade.Section.Name, &grade.Section.Class.Name,
